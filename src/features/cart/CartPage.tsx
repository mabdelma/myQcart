              ...
              <div key={item.menuItem.id} className="bg-white rounded-lg shadow-sm p-4">
                <div className="flex justify-between items-start">
                  <div className="flex-1 min-w-0">
                    ...
                    {renderModifiers(item.menuItem.modifiers)}
                    <div className="flex justify-between items-center mt-2 pt-2 border-t border-gray-100">
                      <span className="text-xs text-gray-400">{item.quantity} × ${item.menuItem.price.toFixed(2)}</span>
                      <span className="text-sm font-semibold text-gray-900">${(item.menuItem.price * item.quantity).toFixed(2)}</span>
                    </div>
                  </div>
