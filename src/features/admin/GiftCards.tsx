...
export default function GiftCards() {
  ...
  if (loading) return <div className="p-4 text-gray-500">Loading gift cards...</div>;
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setShowForm(true)}
          ...
          <Plus className="w-5 h-5 mr-2" /> Create Card
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {cards.map((card) => (
          <div key={card.id} className={`bg-white rounded-lg shadow p-4 border-l-4 ${card.isActive ? 'border-l-green-500' : 'border-l-red-500'}`}>
            ...
            <div className="mt-3 space-y-1 text-sm text-gray-600">
              <p>Balance: <span className="font-semibold">${card.currentBalance.toFixed(2)}</span> / ${card.initialBalance.toFixed(2)}</p>
              {card.expiresAt && <p>Expires: {new Date(card.expiresAt).toLocaleDateString()}</p>}
            </div>
            {card.isActive && (
        ...
        {cards.length === 0 && (
          <p className="text-gray-500 col-span-full text-center py-8">No gift cards created yet.</p>
        )}
      </div>
    </div>
  );
