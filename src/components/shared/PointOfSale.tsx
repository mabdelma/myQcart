...
export function PointOfSale({ role }: PointOfSaleProps) {
  ...
  async function loadData() {
    ...
    try {
      ...
      const mainCategories = menuData.categories.filter((c) => c.type === 'main');
      ...
      if (mainCategories.length > 0) setSelectedCategory(mainCategories[0].id);
      if (occupiedTables.length > 0) setSelectedTable(occupiedTables[0].id);
  ...
  const addToCart = (item: MenuItem) => {
    setCart((current) => {
      ...
      if (existing) return current.map((i) => (i.menuItem.id === item.id ? { ...i, quantity: i.quantity + 1 } : i));
  ...
  const saveNotes = (itemId: string) => {
    setCart((current) => current.map((item) => (item.menuItem.id === itemId ? { ...item, notes: noteText } : item)));
            ...
            {categories.filter((cat) => cat.type === 'main').map((category) => (
              <button key={category.id} onClick={() => setSelectedCategory(category.id)}
                className={`px-4 py-2 rounded-lg whitespace-nowrap ${
                  selectedCategory === category.id ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
            ...
            {filteredItems.map((item) => (
              <button key={item.id} onClick={() => addToCart(item)}
                className={`p-4 rounded-lg border text-left transition-all ${
                  item.available ? 'border-gray-200 hover:border-indigo-500 hover:shadow-md cursor-pointer' : 'border-gray-300 bg-gray-50 opacity-50 cursor-not-allowed'
                ...
                <p className="text-indigo-600 font-medium mt-1">
                  ...
                  {!item.available && <span className="ml-2 text-red-600 text-sm">(Unavailable)</span>}
                </p>
              </button>
            ))}
          </div>
                  ...
                  <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
                    ...
                    <div className="flex justify-end space-x-3 mt-4">
                      ...
                      <button onClick={() => saveNotes(editingNotes)}
                        className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Save</button>
                    </div>
                  </div>
                </div>
              )}
