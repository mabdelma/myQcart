  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      const [allOrders, allTables] = await Promise.all([
        orderApi.getByServer(slug, authState.user.id),
        tableApi.list(slug),
      ]);
      ...
      setMetrics({
        ...
        avgServiceTime: completedOrders.length > 0 ? totalServiceTime / completedOrders.length : 0,
        completionRate:
          ordersWithDetails.length > 0 ? (completedOrders.length / ordersWithDetails.length) * 100 : 0,
        ...
        <div>
          ...
          <div className="mt-2 grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm">
              ...
              <p className="text-xl font-bold text-[#5C4033]">{metrics.avgServiceTime.toFixed(1)} min</p>
            </div>
            <div className="bg-white p-4 rounded-lg shadow-sm">
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm">
              ...
              <p className="text-xl font-bold text-[#5C4033]">${metrics.totalSales.toFixed(2)}</p>
            </div>
          </div>
        </div>
        <button onClick={loadOrders} className="px-4 py-2 bg-[#8B4513] text-white rounded-md hover:bg-[#5C4033]">Refresh History</button>
      </div>

      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1 relative">
          <input type="text" placeholder="Search orders by table or items..." value={searchQuery}
